#!/usr/bin/env python3
"""P02 preregistered true Lamb-defect coherence challenger.

The search protocol is frozen in experiments/P02_preregistered_Lamb_coherence_search.md.
...
def run(random_trials=RANDOM_TRIALS,hill_trials=HILL_TRIALS,optN=24,
        confirm=(24,32,48,64),out='P02_Lamb_coherence_result.json'):
    ...
    if converged: verdict='H-near' if confirms[-1]['kappa']>=0.95 else 'H-gap'
    source=pathlib.Path(__file__).read_bytes()
    result={
      'experiment':'P02','seed':SEED,'basis_wavevectors':WAVEVECTORS.astype(int).tolist(),
      ...
      'scope':'finite-dimensional preregistered true-Lamb-defect challenger; NOT a Navier-Stokes regularity proof'
    }
...
if __name__=='__main__':
    ...
    ap.add_argument('--out',default='P02_Lamb_coherence_result.json')
    args=ap.parse_args()
    if args.quick:
