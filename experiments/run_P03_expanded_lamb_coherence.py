...
def physical_divergences(c, cs, sn, wave, pol1, pol2):
    ...
    for x in range(M):
        ...
        for m in range(wave.shape[0]):
            ...
            A0=c0*pol1[m,0]+c2*pol2[m,0]; A1=c0*pol1[m,1]+c2*pol2[m,1]; A2=c0*pol1[m,2]+c2*pol2[m,2]
            B0=c1*pol1[m,0]+c3*pol2[m,0]; B1=c1*pol1[m,1]+c3*pol2[m,1]; B2=c1*pol1[m,2]+c3*pol2[m,2]
            co,si=cs[m,x],sn[m,x]
            u0 += A0*co+B0*si; u1 += A1*co+B1*si; u2 += A2*co+B2*si
            d0=twopi*(-A0*si+B0*co); d1=twopi*(-A1*si+B1*co); d2=twopi*(-A2*si+B2*co)
            ...
            kk=k0*k0+k1*k1+k2*k2; lapfac=-(twopi*twopi)*kk
            lap0 += lapfac*(A0*co+B0*si); lap1 += lapfac*(A1*co+B1*si); lap2 += lapfac*(A2*co+B2*si)
        om0=g12-g21; om1=g20-g02; om2=g01-g10
        source[x] = -(u0*lap0+u1*lap1+u2*lap2) - (om0*om0+om1*om1+om2*om2)
        rho=math.sqrt(u0*u0+u1*u1+u2*u2)
        if rho>1e-14:
            a0=u0*g00+u1*g01+u2*g02; a1=u0*g10+u1*g11+u2*g12; a2=u0*g20+u1*g21+u2*g22
...
def eval_kappa(c,N,cache=None,details=False):
    ...
    sh=np.fft.fftn(source.reshape(N,N,N)); qh=np.fft.fftn(q.reshape(N,N,N)); normfac=float(N**6)
    n2=float(np.sum(weight*np.abs(sh)**2).real/normfac); g2=float(np.sum(weight*np.abs(qh)**2).real/normfac)
    W=float(np.sum(weight*np.conj(sh)*qh).real/normfac); denom=math.sqrt(max(n2,0.0)*max(g2,0.0))
    kappa=W/denom if denom>0 else 0.0
    if details:
        return kappa, {'kappa':kappa,'W3':W,'QLamb_L2':math.sqrt(max(n2,0.0)),'QG_L2':math.sqrt(max(g2,0.0))}
    return kappa
...
def run(random_trials=RANDOM_TRIALS,hill_trials=HILL_TRIALS,optN=24,confirm=(24,32,48,64),out='P03_expanded_Lamb_coherence_result.json'):
    rng=np.random.default_rng(SEED); cache=make_cache(optN)
    warm=normalize(np.ones(4*WAVEVECTORS.shape[0],dtype=np.float64)); eval_kappa(warm,optN,cache)
    bestk=-2.0; best=None; t0=time.time()
    for j in range(random_trials):
        c=normalize(rng.standard_normal(4*WAVEVECTORS.shape[0])); k=eval_kappa(c,optN,cache)
    ...
    for j in range(hill_trials):
        frac=j/(hill_trials-1) if hill_trials>1 else 1.0; sig=SIGMA0*(SIGMA1/SIGMA0)**frac
        cand=normalize(best+sig*rng.standard_normal(4*WAVEVECTORS.shape[0])); k=eval_kappa(cand,optN,cache)
        if k>bestk: bestk,best=k,cand
        if (j+1)%1000==0: print(f'hill {j+1}/{hill_trials} best={bestk:.9f} sigma={sig:.5f}',flush=True)
    confirms=[]
    for N in confirm:
        kap,d=eval_kappa(best,N,None,True); d['N']=N; confirms.append(d); print('confirm',N,d,flush=True)
    ...
    result={'experiment':'P03','seed':SEED,'basis_wavevectors':WAVEVECTORS.astype(int).tolist(),'random_trials':random_trials,'hill_trials':hill_trials,'optimization_N':optN,'random_best_kappa':random_best,'hill_best_kappa_N24':bestk,'winner_coefficients':best.tolist(),'confirmation':confirms,'convergence_tolerance':2e-4,'converged_last_two':converged,'verdict':verdict,'script_sha256':hashlib.sha256(source).hexdigest(),'elapsed_seconds':time.time()-t0,'scope':'finite-dimensional preregistered expanded true-Lamb-defect challenger; NOT a Navier-Stokes regularity proof'}
...
if __name__=='__main__':
    ap=argparse.ArgumentParser(); ap.add_argument('--quick',action='store_true'); ap.add_argument('--out',default='P03_expanded_Lamb_coherence_result.json'); args=ap.parse_args()
    if args.quick: run(random_trials=20,hill_trials=30,optN=12,confirm=(12,16),out=args.out)
    else: run(out=args.out)
