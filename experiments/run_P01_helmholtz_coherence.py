#!/usr/bin/env python3
"""P01 preregistered Helmholtz-defect coherence challenger.

The search protocol is frozen in experiments/P01_preregistered_Helmholtz_coherence_search.md.
...
def physical_divergences(c, cs, sn, wave, pol1, pol2):
    """Return div((u.grad)u) and div(|u|u) at all grid points.
    ...
    for x in range(M):
        ...
        source[x] = g00*g00+g11*g11+g22*g22 + 2.0*(g01*g10+g02*g20+g12*g21)
...
def eval_kappa(c,N,cache=None,details=False):
    ...
    if details:
        return kappa, {'kappa':kappa,'W3':W,'QN_L2':math.sqrt(max(n2,0.0)),
...
def run(random_trials=RANDOM_TRIALS,hill_trials=HILL_TRIALS,optN=24,
        confirm=(24,32,48,64),out='P01_Helmholtz_coherence_result.json'):
    rng=np.random.default_rng(SEED)
    cache=make_cache(optN)
    # Compile before timing the registered measurement.
    warm=normalize(np.ones(28,dtype=np.float64))
    eval_kappa(warm,optN,cache)
    bestk=-2.0; best=None
    t0=time.time()
    for j in range(random_trials):
        c=normalize(rng.standard_normal(28))
    ...
    for j in range(hill_trials):
        ...
        cand=normalize(best+sig*rng.standard_normal(28))
    ...
    if converged: verdict='H-high' if confirms[-1]['kappa']>=0.90 else 'H-gap'
    source=pathlib.Path(__file__).read_bytes()
    result={
      'experiment':'P01','seed':SEED,'basis_wavevectors':WAVEVECTORS.astype(int).tolist(),
      ...
      'scope':'finite-dimensional preregistered computational challenger; NOT a Navier-Stokes regularity proof'
    }
...
if __name__=='__main__':
    ...
    ap.add_argument('--out',default='P01_Helmholtz_coherence_result.json')
    args=ap.parse_args()
    if args.quick:
